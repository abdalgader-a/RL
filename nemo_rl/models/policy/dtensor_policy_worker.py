...
from nemo_automodel.components._transformers.utils import sliding_window_overwrite as sliding_window_overwrite_V2
from nemo_automodel.components.distributed.cp_utils import (
    create_context_parallel_ctx as create_context_parallel_ctx_V2,
    get_train_context as get_train_context_V2,
)
from nemo_automodel.components.distributed.grad_utils import (
    clip_grad_by_total_norm_ as clip_grad_by_total_norm__V2,
    get_grad_norm as get_grad_norm_V2,
)
from nemo_automodel.components.distributed.parallelizer import (
    fsdp2_strategy_parallelize,
    unshard_fsdp2_model as unshard_fsdp2_model_V2,
)
from nemo_automodel.components.distributed.tensor_utils import (
    get_cpu_state_dict as get_cpu_state_dict_V2,
    to_local_if_dtensor as to_local_if_dtensor_V2,
)
...
class DTensorPolicyWorkerV2:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Manually broadcast buffers
        for _, buf in self.model.named_buffers():
            torch.distributed.broadcast(to_local_if_dtensor_V2(buf), src=0)

        if self.cpu_offload:
            self.model = self.move_to_device(self.model, "cpu")

        if init_reference_model:
            self.reference_model_state_dict = get_cpu_state_dict_V2(
                self.model.state_dict().items(), pin_memory=True
            )
            ...
            for gb_idx in range(num_global_batches):
                ...
                for mb_idx, mb in enumerate(
                    ...
                    if self.cp_size > 1:
                        ...
                        # Create context parallel context
                        context_parallel_ctx = create_context_parallel_ctx_V2(
                            cp_mesh=self.cp_mesh,
                            cp_buffers=cp_buffers,
                            cp_seq_dims=[sequence_dim] * len(cp_buffers),
                            cp_no_restore_buffers=set(cp_buffers),
                    ...
                    with get_train_context_V2(False, False, context_parallel_ctx)():
                        with torch.autocast(device_type="cuda", dtype=self.dtype):
                            outputs = self.model(
                                input_ids=input_ids,
                                attention_mask=attention_mask,
    ...
    def get_logprobs(
        ...
        with unshard_fsdp2_model_V2(self.model), torch.no_grad():
            data.to("cuda")
            dummy_iterator = iter([])
            if self.cfg["dynamic_batching"]["enabled"]:
                mb_iterator = data.make_microbatch_iterator_with_dynamic_shapes()
